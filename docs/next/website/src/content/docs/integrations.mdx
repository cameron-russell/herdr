...
| Session identity | Claude Code, Codex, GitHub Copilot CLI, Devin CLI, Droid, Qoder CLI, Qwen Code, Cursor Agent CLI, Hermes Agent, Antigravity CLI, Grok CLI, Auggie | The integration reports native session references for restore. State still comes from Herdr's screen manifest detection. |
...
Install the Auggie hook:

```bash
herdr integration install auggie
```

The hook reports session identity through Auggie's `SessionStart` hook while Auggie runs inside a Herdr pane. Auggie state comes from Herdr's screen manifest detection.

Herdr uses `~/.augment`, creating it when missing. Install writes `herdr-agent-state.sh` (or `herdr-agent-state.ps1` on Windows) and adds a Herdr `SessionStart` command entry to `settings.json`, preserving any hooks you configured for other events. Uninstall removes the matching hook entry and deletes the hook script.

After Auggie emits a session start event, Herdr can use the reported conversation id to resume the pane with `auggie --resume <id>`.
