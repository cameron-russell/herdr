...
use super::targets::{
    install_antigravity_cli, install_auggie, install_claude, install_codex, install_copilot,
    ...
    install_qwen, uninstall_antigravity_cli, uninstall_auggie, uninstall_claude, uninstall_codex,
...
fn install_target_inner(target: crate::api::schema::IntegrationTarget) -> io::Result<Vec<String>> {
    ...
    let mut messages = match target {
        ...
        crate::api::schema::IntegrationTarget::Auggie => {
            let installed = install_auggie()?;
            vec![
                format!(
                    "installed auggie integration hook to {}",
                    installed.hook_path.display()
                ),
                format!(
                    "ensured auggie hooks at {}",
                    installed.settings_path.display()
                ),
            ]
        }
...
pub(crate) fn uninstall_target(
    ...
    let messages = match target {
        ...
        crate::api::schema::IntegrationTarget::Auggie => {
            let result = uninstall_auggie()?;
            let mut messages = Vec::new();
            if result.removed_hook_file {
                messages.push(format!(
                    "removed auggie hook at {}",
                    result.hook_path.display()
                ));
            } else {
                messages.push(format!(
                    "no auggie hook found at {}",
                    result.hook_path.display()
                ));
            }
            if result.updated_settings {
                messages.push(format!(
                    "removed herdr auggie hook entries from {}",
                    result.settings_path.display()
                ));
            } else {
                messages.push(format!(
                    "no herdr auggie hook entries found in {}",
                    result.settings_path.display()
                ));
            }
            messages
