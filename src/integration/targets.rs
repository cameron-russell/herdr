...
use super::env::{
    antigravity_cli_dir, auggie_dir, claude_dir, codex_dir, copilot_dir, cursor_dir, devin_dir,
...
use super::types::{
    AntigravityCliInstallPaths, AntigravityCliUninstallResult, AuggieInstallPaths,
    AuggieUninstallResult, ClaudeInstallPaths, ClaudeUninstallResult, CodexInstallPaths,
...
use super::{
    ...
    ANTIGRAVITY_CLI_HOOK_INSTALL_NAME, ANTIGRAVITY_CLI_HOOK_TIMEOUT_SEC, AUGGIE_HOOK_ASSET,
    AUGGIE_HOOK_EVENTS, AUGGIE_HOOK_INSTALL_NAME, AUGGIE_HOOK_TIMEOUT_MS, CLAUDE_HOOK_ASSET,
...
pub(crate) fn install_auggie() -> io::Result<AuggieInstallPaths> {
    let dir = auggie_dir()?;
    fs::create_dir_all(&dir)?;

    let hook_path = dir.join(AUGGIE_HOOK_INSTALL_NAME);
    fs::write(&hook_path, AUGGIE_HOOK_ASSET)?;
    make_executable(&hook_path)?;
    ...
    let hooks = ensure_hooks_object(
        ...
        &settings_path,
        "auggie settings",
        "auggie settings hooks",
    )?;
    for (event, action) in AUGGIE_HOOK_EVENTS {
        remove_hook_commands(hooks, event, &hook_path, Some(action))?;
    }
    for (event, action) in AUGGIE_HOOK_EVENTS {
        ensure_command_hook(
    ...
    Ok(AuggieInstallPaths {
        hook_path,
        settings_path,
    })
}

pub(crate) fn uninstall_auggie() -> io::Result<AuggieUninstallResult> {
    let dir = auggie_dir()?;
    let hook_path = dir.join(AUGGIE_HOOK_INSTALL_NAME);
    let settings_path = dir.join("settings.json");
